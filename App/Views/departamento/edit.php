        ...
        <form method="POST" class="border p-4 rounded shadow-sm">
                ...
                <select name="pais_id" class="form-select" required>
                    ...
                    <?php foreach ($paises as $pais): ?>
                        <option value="<?= $pais['pais_id'] ?>"
                            <?= $pais['pais_id'] == $departamento['pais_id'] ? 'selected' : '' ?>>
                            <?= htmlspecialchars($pais['descripcion']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <input type="text" name="descripcion" class="form-control"
                    value="<?= htmlspecialchars($departamento['descripcion']) ?>" required>
            </div>
            <div class="mb-3">
                <label class="form-label">Comida típica:</label>
                <input type="text" name="comida_tipica" class="form-control"
                    value="<?= htmlspecialchars($departamento['comida_tipica']) ?>" required>
            </div>
